...
QETApp::QETApp(QWidget *parent) : QMainWindow(parent) {
	...
	qdw_pa -> setWidget(pa = new PanelDevice(qdw_pa));
...
void QETApp::systrayReduire() {
	...
	hide();
	// on ajoute le menu "Restaurer" et on enl�ve le menu "Masquer"
...
void QETApp::systrayRestaurer() {
	...
	show();
	// on ajoute le menu "Masquer" et on enl�ve le menu "Restaurer"
...
void QETApp::actions() {
	...
	f_reorganise      = new QAction(                               tr("Arranger les fen\352tres r�duites"),this);
	...
	f_reorganise      -> setStatusTip(tr("Aligne les fen\352tres r�duites"));
...
bool QETApp::ouvrir() {
	...
	foreach (QMdiSubWindow/*QWidget*/ *fenetre, workspace.subWindowList/*windowList*/()) {
    ...
    SchemaVue *fenetre_en_cours = qobject_cast<SchemaVue *>(fenetre);
		if (QFileInfo(fenetre_en_cours -> nom_fichier).canonicalFilePath() == chemin_fichier) {
			workspace.setActiveSubWindow/*setActiveWindow*/(fenetre);
			return(false);
		}
	}
...
SchemaVue *QETApp::schemaEnCours() {
    ...
    return activeSubWindow ? qobject_cast<SchemaVue*>(activeSubWindow->widget()) : 0;

	//return(qobject_cast<SchemaVue *>(workspace.activeSubWindow()));
}
...
void QETApp::slot_updateMenuFenetres() {
	...
	// liste des fenetres
	QList<QMdiSubWindow *> fenetres = workspace.subWindowList();
	if (!fenetres.isEmpty()) menu_fenetres -> addSeparator();
    foreach (QMdiSubWindow *pWindow, workspace.subWindowList())
    {
       	SchemaVue *sv = qobject_cast<SchemaVue*>(pWindow->widget());
    ...
    /*
	QList<QWidget *> fenetres = workspace.subWindowList();
	if (!fenetres.isEmpty()) menu_fenetres -> addSeparator();
